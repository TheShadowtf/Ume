...
void ParticleSystem::OnUpdate(Ume::Timestep ts)
{
...
void ParticleSystem::OnRender(Ume::OrthographicCamera& camera)
{
	Ume::Renderer2D::BeginScene(camera);
	for (auto& particle : m_ParticlePool)
	{
		if (!particle.Active)
			continue;
		...
		Ume::Renderer2D::DrawQuad(particle.Position, {size, size}, color, glm::vec1(particle.Rotation));
	}
